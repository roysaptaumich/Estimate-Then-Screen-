...
for (delta in delta_list){
  ...
  error_list = foreach (i = 1:nrep, .packages = c("MASS", "Matrix", "picasso", "glmnet")) %dopar% {
    ...
    ## IHT + screening 
    iht_screening_result = iht_cv_grid_screening_select(X = X, Y= Y,  supp_true = supp_true, beta0 = rep(0,p), gamma = gamma, a = a, v = 1, maxiter = 1e3, prec = 1e-7)
    ...
    ## marginal screening 
    ms_result = ms_ordered_select (X = X, Y = Y, supp_true = supp_true, scale_X = F, s_level = s_hat)
    ...
    return(list(FDR_lasso_rep = FDR_lasso_rep, TPR_lasso_rep = TPR_lasso_rep,
                ...
                prop_recovery_iht_screening_rep = prop_recovery_iht_screening_rep))
  ...
  prop_recovery_lasso_list = c(prop_recovery_lasso_list, apply(do.call(rbind, error_list2[,7]), 2, mean))
  prop_recovery_ms_list = c(prop_recovery_ms_list, apply(do.call(rbind, error_list2[,8]), 2, mean))
  prop_recovery_iht_screening_list = c(prop_recovery_iht_screening_list, apply(do.call(rbind, error_list2[,9]), 2, mean))
}
...
df <- data.frame(r = (1 + delta_list) , FDR_lasso = FDR_lasso, FDR_ms_list = FDR_ms_list, FDR_iht = FDR_iht_screening_list,  FDR_lasso_sd = FDR_lasso_sd, FDR_ms_list_sd = FDR_ms_list_sd, FDR_iht_sd = FDR_iht_screening_list_sd,
                 ...
                 prop_recovery_iht_screening_list = prop_recovery_iht_screening_list)
w = 1.96/sqrt(nrep)
...
save_df <- function (df, q0){
  if (q0 == 1){
    write.csv(df, file = "/home/roysapta/IHT/cv_glmnet/df_fdr_tpr_iht_ms_lasso_CV_homo_p1000_200_rep.csv", row.names = F ) }
  else {
    write.csv(df, file = "/home/roysapta/IHT/cv_glmnet/df_fdr_tpr_iht_ms_lasso_CV_hetero_p1000_200_rep.csv", row.names = F)
  }
}

write.csv(df, file = "/home/roysapta/IHT/cv_glmnet/df_fdr_tpr_iht_ms_lasso_CV_hetero-type2_p1000_200_rep.csv", row.names = F)
#save_df(df, q0)
