...
q0 = 0 # number of spiky signlas = 0, 1, 2, 3
snr = 6

gamma = 10 # no partiton in IHT: gamma = 10
nrep = 200
...
for (k1 in 1:length(r_list)){
  ...
  df = foreach(i = 1:nrep, .packages = c("MASS", "Matrix", "picasso", "glmnet","foreach", "lars", "parallel"), .combine = '+')%dopar%{
    set.seed(i)
  ...
  save_df <- function (df, q0){
    ...
    my_path = paste("/home/roysapta/IHT/cv_glmnet/everything_cv/varying_heterogeneity/error_path/iht_one/iht_no_partition/q_spiky/iht_one_lasso_ms_r", r, "s", s,"p", p, "spiky", q0, "grad-step", eta0,"nrep",nrep,".csv", sep = "_")
