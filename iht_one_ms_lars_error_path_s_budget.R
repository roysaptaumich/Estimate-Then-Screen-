...
for (k1 in 1:length(r_list)){
  ...
  save_df <- function (df, q0){
    if (q0 == 1){
      my_path = paste("/home/roysapta/IHT/cv_glmnet/everything_cv/varying_heterogeneity/error_path/iht_one/homo/homo_lars_error_path_r", r, "s", s,"p", p, "nrep",nrep,".csv", sep = "_")
      write.csv(df, file = my_path, row.names = F ) }
    else {
      my_path = paste("/home/roysapta/IHT/cv_glmnet/everything_cv/varying_heterogeneity/error_path/iht_one/hetero/hetero_lars_error_path_r", r, "s", s,"p", p, "nrep",nrep,".csv", sep = "_")
      write.csv(df, file = my_path, row.names = F )}
  }
  save_df(df, q0)
}
