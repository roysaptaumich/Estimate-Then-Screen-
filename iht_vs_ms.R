...
source("/Users/roysapta/Dropbox (University of Michigan)/Variable Selection (Ziwei and Saptarshi)/R codes/IHT_screening/IHT.R")
source("/Users/roysapta/Dropbox (University of Michigan)/Variable Selection (Ziwei and Saptarshi)/R codes/IHT_screening/IHT_screening.R")

## parameters
p = 1000 
k = 0.9 # sample size parameter
delta_list = seq(0.1, 3, 0.1) # signal streangth paramter r = 1 + delta
l = length(delta_list)
...
for (delta in delta_list){
  ...
  q0 = 1 # probability of the weak signal
  ...
  lambda_list_lasso <- lambda_select_lasso(n = n, mu_x = rep(0,p), S_x = S_x, 
                                           s = s, betamin = betamin, sigma_e = sigma_e, ntest = 1, q = q0)
  
  
  
  error_list = foreach (i = 1:nrep, .packages = c("MASS", "Matrix", "picasso", "glmnet")) %dopar% {
    
    
    ## generate data
    data_i <- data_lm(n = n, mu_x = rep(0,p), S_x = S_x,
                      s = s, betamin = betamin, sigma_e = sigma_e, q = q0)
    X = data_i$X
    Y = data_i$Y  
    beta = data_i$beta
    ...
    ## IHT + screening 
    iht_screening_result = iht_screening_select(X = X, Y= Y, s_list = s  , g_list = s , supp_true = supp_true, beta0 = rep(0,p), gamma = gamma, a = a, v = 1, maxiter = 1e3, prec = 1e-7)
    ...
    ## marginal screening 
    ms_result = ms_ordered_select (X = X, Y = Y, supp_true = supp_true, scale_X = F)
    TPR_ms_list_rep = ms_result$TPR
    FDR_ms_list_rep = ms_result$FDR
...
library(plotly)
df <- data.frame(r = (1 + delta_list) , FDR_lasso = FDR_lasso, FDR_ms_list = FDR_ms_list, FDR_iht = FDR_iht_screening_list,  FDR_lasso_sd = FDR_lasso_sd, FDR_ms_list_sd = FDR_ms_list_sd, FDR_iht_sd = FDR_iht_screening_list_sd,
                 TPR_lasso = TPR_lasso, TPR_ms_list = TPR_ms_list, TPR_iht = TPR_iht_screening_list, TPR_lasso_sd = TPR_lasso_sd, TPR_ms_list_sd = TPR_ms_list_sd, TPR_iht_sd = TPR_iht_screening_list_sd)

#, TPR_iht_one = TPR_iht_one_screening_list ) , FDR_iht_one = FDR_iht_one_screening_list

# write.csv(df, file = "/Users/roysapta/Dropbox (University of Michigan)/Variable Selection (Ziwei and Saptarshi)/R codes/IHT_screening/csv_files/df_fdr_tpr_iht_ms_lasso_hetero_100_rep.csv", row.names = FALSE)

#write.csv(df, file = "/Users/roysapta/Dropbox (University of Michigan)/Variable Selection (Ziwei and Saptarshi)/R codes/IHT_screening/csv_files/df_fdr_tpr_iht_ms_lasso_homo_100_rep.csv", row.names = FALSE)
...
fig2 <- plot_ly(data = df, x = ~r) %>%
  ...
  add_trace(y = ~FDR_iht_screening_list, mode = 'lines', name = 'ETS (FDR)', type = 'scatter', line = list(color = "green"))%>%
...
fig3 <- plot_ly(data = df, x=~r) %>%
  ...
  add_trace(y = ~TPR_iht_screening_list, mode = 'lines', name = 'ETS (TPR)', type = 'scatter', line = list(color = "green"))%>%
